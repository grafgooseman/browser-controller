...
(function() {
    ...
    async function handleCommand(request) {
        try {
            ...
            if (!request.command) {
                console.error('Unknown or undefined command received:', request);
                throw new Error('Unknown command: undefined');
            }
            switch (request.command) {
                ...
                case 'pressKey':
                    return await handlePressKey(request.key);
                default:
                    console.error('Unknown command:', request.command, request);
    ...
    async function handleType(selector, text) {
        ...
        if (element.isContentEditable) {
            ...
            return { success: true, message: `Typed (contenteditable) "${text}" into ${selector}` };
        } else {
    ...
    // Helper for contenteditable typing
    async function typeIntoContentEditable(element, text) {
        element.focus();
        // Clear existing content
        element.innerText = '';
        for (const char of text) {
            document.execCommand('insertText', false, char);
            await humanSim.sleep(Math.random() * 80 + 40);
        }
        element.dispatchEvent(new Event('input', { bubbles: true }));
        element.dispatchEvent(new Event('change', { bubbles: true }));
    }
    ...
    async function handlePressKey(key) {
        // Find the currently focused element or use document.body
        ...
        await humanSim.sleep(Math.random() * 50 + 30);
        ...
        // For Enter key, also trigger form submission if in a form
        if (key === 'Enter' && targetElement.form) {
            await humanSim.sleep(Math.random() * 100 + 50);
